...
metaphor migration generate Invoice budget
metaphor migration run
...
(`budget` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed budget          # run Rust seeders
metaphor migration generate-seeds budget  # emit SQL seed files
```
