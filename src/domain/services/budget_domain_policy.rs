//! Domain policy for Budget aggregate
//!
...
/// Domain policy for Budget — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Budget>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BudgetDomainPolicy = PermitAllPolicy<Budget>;
