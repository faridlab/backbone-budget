//! Domain policy for BudgetLine aggregate
//!
...
/// Domain policy for BudgetLine — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BudgetLine>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BudgetLineDomainPolicy = PermitAllPolicy<BudgetLine>;
